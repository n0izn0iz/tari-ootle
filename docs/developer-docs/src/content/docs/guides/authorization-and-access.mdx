...
- `rule!(caller_template(address))` — the caller of a component method belongs to the given template.
...
- `caller_component(address)` / `caller_template(address)` ask _"is the caller this component/template?"_.
  These are meaningful on **component method** access rules: the caller is the component/template whose
  frame was current immediately before the callee's frame was pushed. `caller_template(address)` matches a
  component instance of that template **or** a static (template-function) call from that template, because
  a static function has a template identity but no component instance.

A method invoked directly by a top-level transaction instruction has no caller component/template frame —
the caller is a signer — so `caller_component` / `caller_template` rules **deny** it. Use a badge, public
...
execution scope, while `caller_component(...)` / `caller_template(...)` refer to the method's caller (see
